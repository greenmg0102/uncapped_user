...
import GradientUserItem from './GradientUserItem'
...
const GradientUser = ({ userTab, userInfoResult, globalFre, reportingResult, setInterestingPair, bufferSetReportItemActive, reportItemActive }: any) => {
        ...
        <div>
            {
                Object.keys(reportingResult).length > 0 &&
                    ...
                    <div className='flex justify-between items-start'>
                        {
                            Object.keys(reportingResult).length > 0 ?
                                <div className='w-full flex justify-center items-center flex-wrap pr-1'>
                                    {
                                        userInfoResult !== undefined &&
                                            Object.keys(userInfoResult).length > 0 ?
                                            playCardArray.map((item, index) =>
                                                ...
                                                >
                                                    <GradientUserItem
                                                        ...
                                                        userTab={userTab}
                                                        standard={standard}
                                                        nextObject={nextObject[item]}
                                                        handResult={userInfoResult[item]}
                                                        reportItemActive={reportItemActive}
                                                        reportingResultItem={reportingResult[item]}
                                                        subValue={userInfoResult[item] && userInfoResult[item].frequency && Object.values(userInfoResult[item].frequency).reduce((acc: any, val: any) => acc + val, 0)}
