...
export default function VillianPokerTable({ villianPokerTable, actionLit, SqueezeStackDepth }: any) {
    ...
    return (
        <div className="flex justify-center items-center h-[195px] w-full max-w-[600px]">
            <div className="relative w-[calc(100%-200px)] flex justify-center items-center flex-col border border-[2px] border-gray-600 rounded-full h-[70px]">
                ...
                <p className={clsx(SqueezeStackDepth !== undefined ? "fond-bold text-[14px] text-gray-200" : "hidden")}>{SqueezeStackDepth} bb</p>
                ...
                {villianPokerTable.map((item: any, index: any) =>
                    <div
                        ...
                        className={
                            clsx(
                                "absolute w-[50px] h-[50px] rounded-[50px] flex flex-justify-center items-center flex-col top-1/2 transform -translate-x-1/2 -translate-y-1/2 border border-[2px] border-gray-800 cursor-pointer hover:border-gray-200 hover:bg-gray-900 hover:text-gray-100 transition-all",
                                actionLit.filter((actionItem: any) => actionItem.action === "H").length > 0 &&
                                    item.title === actionLit.filter((actionItem: any) => actionItem.action === "H")[0].position ? "border-yellow-500" : "border-gray-800"
                            )
                        }
                        style={{
                            top: `${item.gps.top}%`,
                    ...
                    >
                        <div className='w-full h-[25px] pt-[6px]'>
                            <p className="text-center text-gray-300 text-[11px]">{item.title}</p>
                        </div>
                        ...
                        {extractActionList(item.title).length === 1 ?
                            <div className='w-full h-[25px] flex justify-center items-center'>
                                <div
                                    className={
                                        clsx(
                                            'w-full h-[25px] border border-gray-800 border-l-[0px] border-b-[0px] border-r-[0px] rounded-bl-full text-center pt-[0px] font-bold text-gray-400 text-[12px]',
                                            extractActionList(item.title) &&
                                                extractActionList(item.title)[0] &&
                                                extractActionList(item.title)[0].action === "F" ? "text-gray-500" : extractActionList(item.title) &&
                                                    extractActionList(item.title)[0] &&
                                                    extractActionList(item.title)[0].action === "C" ? "text-green-500" : extractActionList(item.title) &&
                                                        extractActionList(item.title)[0] &&
                                                        extractActionList(item.title)[0].action === "H" ? "text-blue-500" : "text-red-500"
                                        )
                                    }
                                >
                                    {
                            ...
                            <div className='w-full h-[25px] flex justify-between items-center'>
                                <div
                                    className={
                                        clsx(
                                            'w-[25px] h-[25px] border border-gray-800 border-l-[0px] border-b-[0px] rounded-bl-full text-right pr-[6px] pt-[0px] font-bold text-gray-400 text-[12px]',
                                            extractActionList(item.title) &&
                                                extractActionList(item.title)[0] &&
                                                extractActionList(item.title)[0].action === "F" ? "text-gray-500" : extractActionList(item.title) &&
                                                    extractActionList(item.title)[0] &&
                                ...
                                <div
                                    className={
                                        clsx(
                                            'w-[25px] h-[25px] border border-gray-800 border-l-[0px] border-b-[0px] border-r-[0px] rounded-bl-full text-left pl-[6px] pt-[0px] font-bold text-gray-400 text-[12px]',
                                            extractActionList(item.title) &&
                                                ...
                                                extractActionList(item.title)[1].action === "F" ? "text-gray-500" : extractActionList(item.title) &&
                                                    ...
                                                    extractActionList(item.title)[1].action === "C" ? "text-green-500" : extractActionList(item.title) &&
                                                        extractActionList(item.title)[1] &&
                                                        extractActionList(item.title)[1].action === "H" ? "text-blue-500" : "text-red-500"
                                        )
                                    }
                                >
                                    {
