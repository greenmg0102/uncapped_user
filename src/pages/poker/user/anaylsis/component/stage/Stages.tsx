...
import BufferNodeItem from './BufferNodeItem'
import StageMenu from './StageMenu'
import Cash from './Cash'
import clsx from 'clsx'
import { analysisTestData } from '../../../../../../utils/reference/analyzeTestData'
import { MiddlewareCurrentOption, MiddlewareReportingOption } from '../../../../../../interface/user/analysis/analysis.dto'

const Stages = ({ nodeList, currentOption, decideNode, activeNode, activeNodeNumber, setCurrentOption, loadingStatus, bettingList, bufferNodeList, bufferAction, isClickable, reportingOption, setReportingOption, reset }: any) => {

    const [stageStatus, setStageStatus] = useState(false)
    const [whichStage, setWhichStage] = useState(-1)
    ...
    return (
        <div className='flex justify-start items-center overflow-x-scroll'>
            <StageMenu
                ...
                setStageStatus={(bool: any) => setStageStatus(bool)}
            />
            ...
            {nodeList.map((item: any, index: any) =>
                <NodeItem
                    ...
                    stackSize={currentOption.stackSize}
                    active={(stage: any) => setWhichStage(stage)}
                    ...
                    // sequenceData={analysisTestData[index + 1] === undefined ? undefined : analysisTestData[index + 1].sequence[analysisTestData[index + 1].sequence.length - 1].type}
                    // sequenceData={analysisTestData[index + 1] === undefined ? undefined : analysisTestData[index + 1].sequence[analysisTestData[index + 1].sequence.length - 1].type}
                />
            )}
            {
                bufferNodeList.length > 0 &&
                bufferNodeList.slice(1).map((item: any, index: any) =>
                    <BufferNodeItem
                        key={index}
                        order={nodeList.length + index}
                        nodeData={item}
                        whichStage={whichStage}
                        stageStatus={stageStatus}
                        isClickable={isClickable}
